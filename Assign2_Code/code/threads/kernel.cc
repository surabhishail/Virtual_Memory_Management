...
Kernel::Initialize()
{
    ...
    //interrupt->Enable();
    // here we're writing the swapFile logic inside this Kernel intitialise 
    //if(kernel->fileSystem->Create("swap"))
    //swapFile = kernel->fileSystem->Open("swap");

    count_id=1;
    fileSystem->Create("swap");
    swapFile=fileSystem->Open("swap");
    if(swapFile==NULL)
    {
        std::cout<<"could not open swap file"<<endl;
    }
    interrupt->Enable();
    swap_pos=0;
