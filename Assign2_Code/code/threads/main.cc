...
main(int argc, char **argv)
{
    ...
    for (i = 1; i < argc; i++) {
        ...
        Proglist.push_back(userProgName);
        i++;
    ...
    if (userProgName != NULL) {
      ...
      //Implemented  changes to let multiple program to run
      for(int i =0; i<Proglist.size();i++){

        Thread *t = NULL;
		cout << "Thread Created for the Execution of Program\n";
        t = new Thread("Programlist");
        t->Fork((VoidFunctionPtr) RunUserProg, (void *) Proglist[i]);
      }
    }
