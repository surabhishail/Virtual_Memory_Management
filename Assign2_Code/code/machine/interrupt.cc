...
void
Interrupt::changeInterrupt(int t)
{
	ListIterator<PendingInterrupt *> *iter = new ListIterator<PendingInterrupt *>(pending);
	PendingInterrupt* p;
	while( !iter->IsDone()) {
		PendingInterrupt* temp= iter->Item();
		iter->Next();
		if(temp->type==TimerInt)
		{
			pending->Remove(temp);
			p=temp;
		}
	...
	p->when=kernel->stats->totalTicks+t;
	pending->Insert(p);
	delete iter;
}
