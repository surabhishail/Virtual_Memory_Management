...
// from the virtual address
	...
	//Code added for Virtual Memory implementation
	int phyPageNumber = -1;
	if (tlb == NULL)
	{
		int pages = 0;

		while (pages < NumPhysPages)
		{
			if (pageTable[pages].threadId == kernel->currentThread
				&& pageTable[pages].virtualPage == vpn)
			{
				entry = &(pageTable[pages]);
				phyPageNumber = pages;
				pages = NumPhysPages;
			}
			pages++;
		}
		if (phyPageNumber == -1 || !entry->valid)
		{
			std::cout << "Page Fault Exception Occured	" << endl;
